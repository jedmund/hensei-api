...
module Granblue
  module Parsers
    class WeaponSkillParser
      ...
      # Per-modifier icon→(series,size) catalog (data/weapon_skill_icon_map.json),
      # generated by `rake granblue:extract_weapon_skill_icon_map`.
      def self.icon_map
        @icon_map ||= begin
          path = Rails.root.join("data", "weapon_skill_icon_map.json")
          File.exist?(path) ? JSON.parse(File.read(path)) : {}
        end
      end

      # Resolve (series, size) for a skill version from its stored icon, using the
      # catalog. The icon is the most reliable source (it encodes both). Returns
      # { series:, size: } or nil when the icon isn't in the modifier's catalog.
      def self.derive_from_icon(modifier, icon)
        return nil unless modifier && icon.present?
        entries = icon_map[modifier]
