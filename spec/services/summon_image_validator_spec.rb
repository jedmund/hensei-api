...
RSpec.describe SummonImageValidator do
  let(:valid_id) { '2040001000' }
  ...
  describe '#valid?' do
    ...
    context 'with valid format' do
      ...
      it 'handles timeouts gracefully' do
        allow(http_double).to receive(:request).and_raise(Net::ReadTimeout, 'timed out')
        expect(validator.valid?).to be false
  ...
  describe '#exists_in_db?' do
    it 'checks Summon table for granblue_id' do
      allow(Summon).to receive(:exists?).with(granblue_id: valid_id).and_return(false)
      validator = described_class.new(valid_id)
      expect(validator.exists_in_db?).to be false
    end
  end
end
